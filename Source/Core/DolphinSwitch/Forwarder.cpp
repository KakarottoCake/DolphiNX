...
#include "DolphinSwitch/ForwarderStructs.h"
...
struct BufHelper {
    ...
    {
        if (where_to > std::numeric_limits<size_t>::max()) {
            valid = false;
            return;
        }
        offset = where_to;
...
bool add_file_entry(FileEntries &entries, const char *name, const void *data, u64 size)
{
...
void copy_nacp_text(char *destination, size_t destination_size, const std::string &source)
{
    ...
    size_t length = std::min(clean.size(), destination_size - 1);
    if (length < clean.size()) {
        // Do not leave an incomplete UTF-8 sequence at the field boundary.
        while (length > 0 &&
               (static_cast<unsigned char>(clean[length]) & 0xc0) == 0x80)
            --length;
    }
...
std::string resolveLauncherPath()
{
    ...
    if (!s_self_path.empty())
        candidates.push_back(s_self_path);
    candidates.emplace_back(kLauncherNro);

    for (std::string path : candidates) {
...
struct ForwarderConfigTransaction
{
    ...
    Result stage(u64 tid, const std::string &nroPath, const std::string &args)
    {
        ...
        if (mkdir("sdmc:/switch/dolphin/forwarders", 0777) != 0 &&
            (errno != EEXIST || stat("sdmc:/switch/dolphin/forwarders", &directoryStat) != 0 ||
                         ...
                         const std::vector<u8> &iconJpeg, ForwarderStage &stage)
{
    ...
    {
        FileEntries exefs;
        if (!add_file_entry(exefs, "main", nsoData.data(), nsoData.size()) ||
            !add_file_entry(exefs, "main.npdm", npdmData.data(), npdmData.size()))
            return kForwarderIoError;
        NcaEntry program = create_program_nca(tid, headerKey, exefs);
        if (!program.valid)
            return kForwarderIoError;
    ...
    }
    {
        ...
        if (!add_file_entry(romfs, "/control.nacp", &nacp, sizeof(nacp)) ||
            ...
            return kForwarderIoError;
        NcaEntry control = create_control_nca(tid, headerKey, romfs);
        if (!control.valid)
            return kForwarderIoError;
        ncaEntries.emplace_back(std::move(control));
    }
                     ...
                     const std::string &iconImgPath, char *err, std::size_t errSize)
{
    if (titleId == 0) {
        if (err && errSize)
            snprintf(err, errSize, "The installed Wii title ID is invalid.");
        return false;
    }
